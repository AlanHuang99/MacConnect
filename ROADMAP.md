...
- Sleep/wake + Wi-Fi-change recovery: discovery rebuilds and stale links drop on `NSWorkspace.didWake` and `NWPathMonitor` interface changes, so peers reconnect without an app restart; the popover refresh button now does a full re-discover (0.3.5).
